...
export default function ForgotPassword() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Não foi possível solicitar a recuperação agora")
          ...
          {sent ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-[#6b7280] leading-relaxed mt-2">
                Se existir uma conta com esse endereço, enviaremos um link para
                redefinir sua senha.
              </p>
              <Link
                to="/"
                className="inline-block mt-7 text-sm font-semibold text-[#16a34a] hover:underline"
              >
                Voltar ao login
              </Link>
            </div>
