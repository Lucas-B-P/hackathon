...
export default function BanhoTosa() {
  ...
  const statuses = Object.fromEntries(
    pets.map((p) => [p.id, { id: p.id, status: p.status, obs: p.notes ?? "" }]),
  )
  ...
  const advance = async (id: number) => {
    ...
    if (next !== cur) {
      const updated = await updateGroomingStatus(id, next)
      setPets((items) =>
        items.map((item) =>
    ...
    <div className="p-4 md:p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-[#6b7280]">
          Fila de atendimento do dia — {pets.length} pets
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        {FLOW.map((f, i) => (
          <div key={f} className="flex items-center gap-2 flex-shrink-0">
            <span
              className={`text-xs px-3 py-1.5 rounded-full font-medium border ${FLOW_COLORS[f]}`}
            >
              {f}
            </span>
            {i < FLOW.length - 1 && (
          ...
          return (
            <div
              ...
              className={`bg-white rounded-xl border shadow-sm p-5 transition-all ${
                isDone
                  ? "opacity-60 border-[#e5e7eb]"
                  : "border-[#e5e7eb] hover:border-[#86efac] hover:shadow-md"
              }`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-medium border flex-shrink-0 ${FLOW_COLORS[cur] || "bg-gray-100 text-gray-500 border-gray-200"}`}
                    >
                      {cur}
                    </span>
                  </div>
              ...
              <div className="mt-4 space-y-1.5">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium text-[#374151]">
                    {new Date(pet.starts_at).toLocaleTimeString("pt-BR", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </span>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-3 mt-4">
              <button
                ...
                className="flex-1 py-2.5 border border-[#e5e7eb] rounded-xl text-sm text-[#374151] hover:bg-[#f3f4f6]"
              >
                Cancelar
              </button>
              <button
                onClick={() => setObsOpen(null)}
                className="flex-1 py-2.5 bg-[#16a34a] text-white rounded-xl text-sm font-semibold hover:bg-[#15803d]"
                style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
              >
                Salvar
              </button>
