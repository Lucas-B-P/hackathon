...
const todos = [
...

  {
    id: "#1028",
    data: "15/07/2026",
    produtos: "Ração Gatos Whiskas 3kg",
    valor: 49.9,
    status: "Entregue",
  },

  {
    id: "#1055",
    data: "13/08/2026",
    produtos: "Brinquedo Kong Classic",
    valor: 39.9,
    status: "Recebido",
  },
]
    ...
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-[#6b7280]">
          {todos.length} pedidos registrados
        </p>
      </div>
          ...
          <tbody className="divide-y divide-[#f3f4f6]">
            {todos.map((p) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-[#374151] max-w-xs truncate">
                  <span className="block">{p.products}</span>
                  <span className="text-xs text-[#9ca3af]">{p.customer}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <select
                    ...
                    className={`text-xs px-2 py-1 rounded-full font-medium border-0 outline-none ${STATUS_COLORS[p.status] || "bg-gray-100 text-gray-500"}`}
                  >
                    <option>Recebido</option>
                    <option>Em preparacao</option>
