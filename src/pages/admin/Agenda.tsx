...
export default function Agenda() {
  ...
  useEffect(() => {
    getAdminAppointments()
      .then((result) => setAgendamentos(result.data))
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="flex items-center gap-3 bg-white border border-[#e5e7eb] rounded-xl p-3 shadow-sm">
        <button className="p-1.5 hover:bg-[#f3f4f6] rounded-lg text-[#6b7280] transition-colors">
          <ChevronLeft size={18} />
        </button>
        <span
          className="font-semibold text-sm text-[#111827] mx-1"
          style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
        >
          {view === "mes"
            ? `${MONTH} ${YEAR}`
            : `11 – 17 de ${MONTH} de ${YEAR}`}
        </span>
        <button className="p-1.5 hover:bg-[#f3f4f6] rounded-lg text-[#6b7280] transition-colors">
          <ChevronRight size={18} />
        </button>
        <button className="ml-2 text-xs text-[#16a34a] hover:underline font-medium">
          Hoje
        </button>
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
          <div className="grid grid-cols-7 border-b border-[#e5e7eb]">
            {WEEK_DAYS.map((d) => (
              <div
                key={d}
                className="py-2 text-center text-xs font-semibold text-[#9ca3af]"
              >
                {d}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7 divide-x divide-y divide-[#f3f4f6]">
            {calDays.map((day, i) => (
              <div
                key={i}
                className={`min-h-[90px] p-2 text-xs ${
                  day === today ? "bg-[#f0fdf4]" : ""
                } ${!day ? "bg-[#fafafa]" : ""}`}
              >
                {day && (
                  <>
                    ...
                    {day === today && agendamentos.slice(0, 3).map((ag) => (
                        <div
                          key={ag.id}
                          className="text-[10px] bg-[#dcfce7] text-[#15803d] rounded px-1 py-0.5 mb-0.5 truncate"
                        >
                          {ag.horario} {ag.pet}
                        </div>
                      ))}
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
          ...
          <div className="overflow-y-auto max-h-[500px]">
            {HOURS.map((h) => (
              <div
                key={h}
                className="grid border-b border-[#f3f4f6]"
                style={{ gridTemplateColumns: "60px repeat(7,1fr)" }}
              >
                <div className="py-3 px-2 text-[10px] text-[#9ca3af] text-right font-mono">
                  {h}
                </div>
                {Array.from({ length: 7 }, (_, i) => (
                  <div
                    key={i}
                    className={`border-l border-[#f3f4f6] min-h-[52px] p-1 ${
                      i === 3 ? "bg-[#f0fdf4]/50" : ""
                    }`}
                  >
                    {i === 3 &&
                      agendamentos.find((a) => a.horario === h) &&
                      (() => {
                        ...
                        return (
                          <div
                            className={`text-[10px] rounded-lg px-2 py-1.5 border ${cls} leading-tight`}
                          >
          ...
          <div className="overflow-y-auto max-h-[500px]">
            {HOURS.map((h) => {
              ...
              return (
                <div
                  key={h}
                  className="flex border-b border-[#f3f4f6] min-h-[64px]"
                >
                  <div className="w-16 py-3 px-3 text-[11px] text-[#9ca3af] font-mono flex-shrink-0 border-r border-[#f3f4f6]">
                    {h}
                  </div>
                  <div className="flex-1 p-2">
                    {ag && (
                      <div
                        className={`flex items-start gap-3 rounded-xl px-4 py-3 border ${STATUS_COLORS[ag.status]}`}
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs opacity-70">
                            Tutor: {ag.tutor} · Responsável: {ag.funcionario}
                          </p>
                        </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h2
              className="text-lg font-bold text-[#111827] mb-5"
              style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
            >
              Novo Agendamento
            </h2>
            <div className="space-y-4">
              {[
                ...
                ["Funcionário", "Selecionar funcionário"],
              ].map(([label, ph]) => (
                <div key={label}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-[#e5e7eb] rounded-xl text-sm outline-none focus:border-[#16a34a]"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#374151] mb-1">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={() => setModalOpen(false)}
                className="flex-1 py-2.5 bg-[#16a34a] text-white rounded-xl text-sm font-semibold hover:bg-[#15803d] transition-colors"
                style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
              >
                Confirmar
              </button>
