...
export default function Vendas() {
  ...
  const [payModal, setPayModal] = useState<PayModal>({
    open: false,
    metodo: "PIX",
  })
  ...
  const cats = [
    "Todos",
    ...Array.from(new Set(produtos.map((p) => p.category))),
  ]
  ...
  const addToCart = (p: AdminProduct) => {
    setCart((prev) => {
      ...
      return [
        ...prev,
        { id: p.id, nome: p.name, preco: Number(p.sale_price), qty: 1 },
      ]
    })
  }
  ...
  const finalizarVenda = async () => {
    await createAdminSale({
      ...
      formaPagamento: payModal.metodo,
    })
    ...
    <div className="h-full flex">
      ...
      <div className="flex-1 p-4 md:p-6 space-y-4 overflow-y-auto border-r border-[#e5e7eb]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
          {filtered.map((p) => (
            ...
            >
              <p className="text-xs font-semibold text-[#111827] leading-tight group-hover:text-[#15803d] transition-colors">
                {p.name}
              </p>
      ...
      <div className="w-80 flex flex-col bg-white">
        <div className="px-5 py-4 border-b border-[#f3f4f6] flex items-center gap-2">
          ...
          <span className="ml-auto text-xs bg-[#f3f4f6] text-[#374151] px-2 py-0.5 rounded-full font-medium">
            {cart.length} itens
          </span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-2 p-3 bg-[#fafafa] rounded-xl"
              >
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-[#374151] leading-tight truncate">
                    {item.nome}
                  </p>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="w-6 text-center text-xs font-semibold text-[#111827]">
                    {item.qty}
                  </span>
                  <button
                    onClick={() => updateQty(item.id, 1)}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                onClick={() => setPayModal((m) => ({ ...m, open: false }))}
                className="p-1 hover:bg-[#f3f4f6] rounded-lg text-[#9ca3af]"
              >
                <X size={18} />
              </button>
            </div>
