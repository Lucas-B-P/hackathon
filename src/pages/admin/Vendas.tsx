...
export default function Vendas() {
  ...
  return (
    <div className="h-full flex relative">
      ...
      {cart.length > 0 && <button onClick={() => setPayModal({ open: true, metodo: "PIX" })} className="fixed bottom-6 right-6 z-30 flex items-center gap-3 rounded-xl bg-[#16a34a] px-5 py-3 text-sm font-semibold text-white shadow-lg hover:bg-[#15803d]"> <ShoppingCart size={17} /> Finalizar venda ({cart.length}) · R$ {total.toFixed(2)}</button>}
