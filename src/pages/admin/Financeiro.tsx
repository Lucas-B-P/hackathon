...
export default function Financeiro() {
  ...
  useEffect(() => {
    getAdminFinance()
      .then((result) => setFinance(result.data))
  ...
  const mapEntry = (item: AdminFinance["entries"][number]) => ({
    ...item,
  ...
  const contasReceber =
    finance?.entries.filter((item) => item.type === "RECEBER").map(mapEntry) ??
    []
  const contasPagar =
    finance?.entries.filter((item) => item.type === "PAGAR").map(mapEntry) ?? []
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#f3f4f6]">
              {(tab === "receber" ? contasReceber : contasPagar).map((c) => {
                const sc =
                  tab === "receber" ? STATUS_CR[c.status] : STATUS_CP[c.status]
