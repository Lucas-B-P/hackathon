...
import { Package, TrendingDown, AlertTriangle, DollarSign, Plus, X } from "lucide-react";
import { createStockMovement, getAdminStock, type AdminStock } from "../../services/api";
...
export default function Estoque() {
  ...
  const [form, setForm] = useState({ productId: "", type: "ENTRADA" as "ENTRADA" | "SAIDA" | "AJUSTE", quantity: "", reason: "" });
  const [error, setError] = useState("");
  const loadStock = () => getAdminStock().then((result) => setStock(result.data)).catch(() => setStock(null));
  useEffect(() => { loadStock(); }, []);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><form onSubmit={async (event) => { event.preventDefault(); try { await createStockMovement({ productId: Number(form.productId), type: form.type, quantity: Number(form.quantity), reason: form.reason }); await loadStock(); setOpen(false); setForm({ productId: "", type: "ENTRADA", quantity: "", reason: "" }); } catch (err) { setError(err instanceof Error ? err.message : "Não foi possível registrar a movimentação"); } }} className="w-full max-w-md space-y-4 rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-lg font-bold text-[#111827]">Registrar movimentação</h2><button type="button" onClick={() => setOpen(false)}><X size={18} className="text-[#9ca3af]" /></button></div><label className="block text-xs font-medium text-[#374151]">Produto<select required value={form.productId} onChange={e => setForm({ ...form, productId: e.target.value })} className="mt-1 w-full rounded-xl border border-[#e5e7eb] bg-white px-3 py-2 text-sm"><option value="">Selecione um produto</option>{produtos.map(p => <option key={p.id} value={p.id}>{p.nome} — estoque: {p.estoque}</option>)}</select></label><label className="block text-xs font-medium text-[#374151]">Tipo<select value={form.type} onChange={e => setForm({ ...form, type: e.target.value as typeof form.type })} className="mt-1 w-full rounded-xl border border-[#e5e7eb] bg-white px-3 py-2 text-sm"><option value="ENTRADA">Entrada</option><option value="SAIDA">Saída</option><option value="AJUSTE">Ajuste para estoque informado</option></select></label><label className="block text-xs font-medium text-[#374151]">Quantidade<input required min="1" type="number" value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} className="mt-1 w-full rounded-xl border border-[#e5e7eb] px-3 py-2 text-sm" /></label><label className="block text-xs font-medium text-[#374151]">Motivo<input value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} placeholder="Ex.: compra, perda ou inventário" className="mt-1 w-full rounded-xl border border-[#e5e7eb] px-3 py-2 text-sm" /></label>{error && <p className="text-sm text-red-600">{error}</p>}<button className="w-full rounded-xl bg-[#16a34a] py-3 font-semibold text-white">Salvar movimentação</button></form></div>}
