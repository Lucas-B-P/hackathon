...
export default function Estoque() {
  ...
  const produtos =
    stock?.products.map((item) => ({
      ...
      sku: item.sku,
      estoque: item.stock,
      estoqueMin: item.minimum_stock,
      custo: Number(item.cost),
    })) ?? []
  const movimentacoesEstoque =
    stock?.movements.map((item) => ({
      id: item.id,
  ...
  const baixo = produtos.filter(
    (p) => p.estoque > 0 && p.estoque <= p.estoqueMin,
  ).length
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div>
          ...
          <p className="text-sm text-[#6b7280]">
            Controle de movimentações e inventário
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={label}
            className="bg-white rounded-xl p-5 border border-[#e5e7eb] shadow-sm flex items-center gap-4"
          >
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#f3f4f6]">
              {["Produto", "Tipo", "Qtd.", "Motivo", "Usuário", "Data"].map(
                (h) => (
                  <th
                    key={h}
            ...
            {movimentacoesEstoque.map((m) => (
              <tr key={m.id} className="hover:bg-[#fafafa] transition-colors">
                ...
                <td className="px-4 py-3 font-mono text-sm">
                  <span
                    className={
                      m.quantidade < 0 ? "text-red-500" : "text-[#111827]"
                    }
                  >
                    {m.quantidade > 0 ? "+" : ""}
                    {m.quantidade}
      ...
      <div className="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden overflow-x-auto">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#f3f4f6]">
              {["Produto", "SKU", "Estoque atual", "Mínimo", "Status"].map(
                (h) => (
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-xs font-semibold text-[#9ca3af]"
