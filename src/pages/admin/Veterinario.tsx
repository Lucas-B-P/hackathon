...
export default function Veterinario() {
  ...
  useEffect(() => {
    getVeterinaryAppointments()
      .then((result) => setConsultas(result.data))
      .catch(() => setConsultas([]))
  }, [])
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#6b7280]">
            Consultas e procedimentos de hoje
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-4">
          ...
          >
            ...
            <div className="space-y-1 border-t border-[#f3f4f6] pt-3">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-[#374151] font-medium">{c.type}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-[#9ca3af]">Horário</span>
                <span className="text-[#374151] font-medium">
                  {new Date(c.starts_at).toLocaleTimeString("pt-BR", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </span>
              ...
              <div className="flex justify-between text-xs">
                <span className="text-[#9ca3af]">Veterinário</span>
                <span className="text-[#374151] font-medium">
                  {c.veterinarian}
                </span>
              </div>
        ...
        {consultas.length === 0 && (
          <div className="col-span-full bg-white rounded-xl border border-dashed border-[#d1d5db] p-10 text-center text-sm text-[#6b7280]">
            Nenhuma consulta veterinária agendada para hoje.
          </div>
        )}
      </div>
    </div>
