...
const KPI = ({
  ...
  const realValue =
    label === "Vendas hoje"
      ? money(dashboardKpis?.salesToday ?? 0)
      : label.includes("Faturamento")
        ? money(dashboardKpis?.monthlyRevenue ?? 0)
        : label === "Agendamentos hoje"
          ? String(dashboardKpis?.appointmentsToday ?? 0)
          : label === "Clientes ativos"
            ? String(dashboardKpis?.activeClients ?? 0)
            : label === "Contas a receber"
              ? money(dashboardKpis?.receivable ?? 0)
              : value
  return (
...
export default function Dashboard() {
  ...
  const totalCategories =
    dashboard?.categories.reduce((sum, item) => sum + Number(item.total), 0) ||
    1
  const vendasCategoria =
    dashboard?.categories.map((item) => ({
      name: item.name,
      value: Math.round((Number(item.total) / totalCategories) * 100),
    })) ?? []
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-[#6b7280] text-sm mt-0.5">
          Aqui está o resumo da operação de hoje, 14 de agosto de 2026 —
          Patinhas Pet Shop.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <KPI
          label="Estoque baixo"
          value={String(estoquesBaixos.length)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="col-span-1 lg:col-span-2 bg-white rounded-xl p-5 border border-[#e5e7eb] shadow-sm">
          ...
          <div className="flex gap-4 mt-2">
            ...
            <span className="flex items-center gap-1.5 text-xs text-[#6b7280]">
              <span className="w-3 h-0.5 bg-[#ef4444] inline-block rounded-full" />
              Despesas
            </span>
          </div>
        </div>
          ...
          <div className="divide-y divide-[#f3f4f6]">
            {agendamentos.slice(0, 5).map((ag) => (
              <div key={ag.id} className="flex items-center gap-3 px-5 py-3">
                <span className="text-xs font-mono text-[#6b7280] w-10 flex-shrink-0">
                  {ag.time}
                </span>
                ...
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${STATUS_COLORS[ag.status] || "bg-gray-100 text-gray-600"}`}
                >
                  {ag.status}
                </span>
              </div>
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#f3f4f6]">
            ...
            <span className="text-xs bg-orange-50 text-orange-600 px-2 py-0.5 rounded-full font-medium">
              {estoquesBaixos.length} produtos
            </span>
          </div>
          <div className="divide-y divide-[#f3f4f6]">
