    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {servicos.map((s) => (
          <div
            key={s.id}
            className="bg-white rounded-xl border border-[#e5e7eb] shadow-sm p-5 hover:border-[#86efac] hover:shadow-md transition-all cursor-pointer"
          >
            ...
            <div className="mt-2">
              <p className="text-[11px] text-[#9ca3af]">
                Responsável:{" "}
                <span className="text-[#374151]">{s.funcionario}</span>
              </p>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              {[
                ["Nome do serviço", "text"],
                ["Descrição", "text"],
                ["Duração (ex: 1h30)", "text"],
              ].map(([label, type]) => (
                <div key={label}>
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => setModalOpen(false)}
                className="flex-1 py-2.5 border border-[#e5e7eb] rounded-xl text-sm text-[#374151] hover:bg-[#f3f4f6] transition-colors"
              >
                Cancelar
              </button>
              <button
