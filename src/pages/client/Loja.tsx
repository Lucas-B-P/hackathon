...
export default function Loja() {
  ...
  useEffect(() => {
    Promise.all([
      getStoreCategories().then((result) => result.data).catch(() => []),
      getStoreProducts().then((result) => result.data).catch(() => []),
  ...
  const addToCart = (product: StoreProduct) => {
    ...
    setCart((prev) => {
      ...
      return [...prev, { id: product.id, nome: product.name, preco: price, qty: 1 }];
    });
  };
  ...
  const selectedAddress = addresses.find((item) => item.id === selectedAddressId) ?? addresses.find((item) => item.is_primary) ?? addresses[0];
  ...
  async function openCheckout() {
    ...
    try {
      ...
      const primary = result.data.find((item) => item.is_primary) ?? result.data[0];
  ...
  async function confirmar() {
    ...
    try {
      const result = await createOrder({
        itens: cart.map((item) => ({ produtoId: item.id, quantidade: item.qty })),
  ...
  return (
    <div className="space-y-6">
          ...
          {cats.map((category) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-xl border transition-all ${
                cat === category ? "bg-[#16a34a] text-white border-[#16a34a]" : "bg-white text-[#6b7280] border-[#e5e7eb] hover:border-[#d1d5db]"
              }`}
            >
      ...
      {loading ? (
        <p className="text-sm text-[#9ca3af] text-center py-20">Carregando produtos...</p>
      ) : products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {products.map((product) => {
            ...
            const price = toNumber(product.sale_price);
            return (
              <div key={product.id} className="bg-white rounded-2xl border border-[#e5e7eb] shadow-sm overflow-hidden hover:shadow-md hover:border-[#86efac] transition-all group">
                ...
                <div className="p-4">
                  <p className="text-sm font-semibold text-[#111827] leading-tight mb-0.5 group-hover:text-[#15803d] transition-colors">{product.name}</p>
                  <p className="text-xs text-[#9ca3af] mb-3">SKU: {product.sku}</p>
                  <div className="flex flex-col gap-2">
                    ...
                    {item ? (
                      <div className="flex items-center justify-between bg-[#f0fdf4] border border-[#86efac] rounded-xl px-3 py-2">
                        <button onClick={() => updateQty(product.id, -1)} className="text-[#16a34a] hover:text-[#15803d] transition-colors">
                          <Minus size={16} />
                        </button>
          ...
          <div className="w-full max-w-md bg-white flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#f3f4f6]">
              ...
              {step === "endereco" && (
                <>
                  ...
                  <span className="font-semibold text-[#111827] text-sm" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                    Entrega
                  </span>
                  <div className="w-14" />
                </>
            ...
            {step !== "confirmado" && (
              <div className="flex items-center px-5 py-2 border-b border-[#f3f4f6] gap-1">
                {[["cart", "Carrinho"], ["endereco", "Entrega"], ["pagamento", "Pagamento"]].map(([value, label], index) => {
                  const steps = ["cart", "endereco", "pagamento"];
                  const current = steps.indexOf(step);
                  const mine = steps.indexOf(value);
                  return (
                    <div key={value} className="flex items-center flex-1 last:flex-none">
                      <span className={`text-[11px] font-medium ${mine <= current ? "text-[#16a34a]" : "text-[#d1d5db]"}`}>{label}</span>
                      {index < 2 && <ChevronRight size={12} className="mx-1 text-[#d1d5db] flex-shrink-0" />}
                    </div>
                  );
                })}
              </div>
            ...
            <div className="flex-1 overflow-y-auto">
                  ...
                  {cart.length === 0 ? (
                    ...
                    cart.map((item) => (
                      <div key={item.id} className="flex items-center gap-3 p-3 bg-[#fafafa] rounded-xl">
                        <div className="w-10 h-10 bg-[#f0fdf4] rounded-xl flex items-center justify-center text-xl flex-shrink-0">🛍️</div>
                ...
                <div className="p-5 space-y-4">
                  {addresses.length === 0 ? (
                    <div className="p-4 border border-amber-200 bg-amber-50 rounded-xl text-sm text-amber-800">
                      ...
                      <Link to="/portal/perfil" className="font-semibold underline">
                        Ir para o perfil
                      </Link>
                    </div>
                  ) : (
                    <>
                        ...
                        {addresses.map((address) => (
                          <button
                            ...
                            className={`w-full p-4 border-2 rounded-xl text-left transition-all ${
                              (selectedAddressId ?? selectedAddress?.id) === address.id ? "border-[#16a34a] bg-[#f0fdf4]" : "border-[#e5e7eb]"
                            }`}
                          >
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-semibold text-[#374151]">{address.label || "Endereço"}</span>
                              {(selectedAddressId ?? selectedAddress?.id) === address.id && <Check size={14} className="text-[#16a34a]" />}
                            </div>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-[#374151] mb-1.5">Observações de entrega</label>
                        <input
                          type="text"
                          value={complemento}
              ...
              {step === "pagamento" && (
                <div className="p-5 space-y-4">
                    ...
                    {PAYMENT.map((method) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all ${
                          pagamento === method ? "border-[#16a34a] bg-[#f0fdf4]" : "border-[#e5e7eb] hover:border-[#d1d5db]"
                        }`}
                      >
                        <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${pagamento === method ? "border-[#16a34a] bg-[#16a34a]" : "border-[#d1d5db]"}`}>
                          {pagamento === method && <div className="w-full h-full rounded-full scale-50 bg-white" />}
                        </div>
                        <span className="text-sm font-medium text-[#374151]">{method}</span>
                        {method === "PIX" && <span className="ml-auto text-[10px] bg-[#dcfce7] text-[#15803d] px-2 py-0.5 rounded-full font-medium">5% off</span>}
                      </button>
                    ))}
                  </div>
                  <div className="bg-[#f9fafb] rounded-xl p-4 space-y-2 border border-[#f3f4f6]">
                    ...
                    {cart.map((item) => (
                      <div key={item.id} className="flex justify-between text-xs text-[#6b7280]">
                        <span className="truncate mr-2">
                          {item.nome} × {item.qty}
                        </span>
                        <span className="flex-shrink-0">R$ {(item.preco * item.qty).toFixed(2)}</span>
                      </div>
                    ))}
                    <div className="pt-2 border-t border-[#e5e7eb] flex justify-between font-bold text-[#111827]">
                      ...
                      <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>R$ {displayTotal.toFixed(2)}</span>
                    </div>
                    {pagamento === "PIX" && <p className="text-[11px] text-[#16a34a] font-medium">Desconto de 5% aplicado no PIX 🎉</p>}
                  </div>
              ...
              {step === "confirmado" && (
                <div className="flex flex-col items-center justify-center h-full py-20 px-6 text-center">
                  ...
                  <p className="text-sm text-[#6b7280] mb-4">Recebemos sua solicitação. Você receberá uma confirmação por e-mail em breve.</p>
                  <div className="bg-[#f9fafb] rounded-xl p-4 w-full border border-[#f3f4f6] text-left space-y-1.5">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-semibold text-[#374151]">#{orderId}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-[#9ca3af]">Pagamento</span>
                      <span className="font-semibold text-[#374151]">{pagamento}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-[#9ca3af]">Prazo</span>
                      <span className="font-semibold text-[#374151]">2 a 4 dias úteis</span>
                    </div>
                  </div>
