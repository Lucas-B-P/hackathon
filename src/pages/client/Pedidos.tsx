...
import { displayOrderStatus, formatDatePt, getOrders, toNumber, type Order } from "../../services/api";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#6b7280]">{loading ? "Carregando..." : `${orders.length} pedidos realizados`}</p>
      </div>
      ...
      {!loading && orders.length === 0 ? (
        ...
        <div className="space-y-3">
          {orders.map((order) => {
            ...
            const Icon = STATUS_ICONS[order.status] || STATUS_ICONS[status] || Package;
            const color = STATUS_COLORS[order.status] || STATUS_COLORS[status] || "bg-gray-100 text-gray-500";
            return (
              <div key={order.id} className="bg-white rounded-2xl border border-[#e5e7eb] shadow-sm p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-bold text-[#111827] font-mono text-sm">#{order.id}</span>
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${color}`}>{status}</span>
                    </div>
                    <p className="text-sm text-[#374151] mt-1">{order.products || "Pedido sem itens"}</p>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-xs text-[#9ca3af]">{formatDatePt(order.created_at)}</span>
                      <span className="font-bold text-[#111827]">R$ {toNumber(order.total).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                      ...
                      {STEPS.map((step, index) => {
                        ...
                        return (
                          <div key={step} className="flex items-center flex-1 last:flex-none">
                            ...
                            {index < 4 && <div className={`flex-1 h-px ${isActive && index < curIdx ? "bg-[#16a34a]" : "bg-[#e5e7eb]"}`} />}
                          </div>
                        );
                      })}
                    </div>
                    <p className="text-xs text-[#16a34a] font-medium mt-1.5">{status}</p>
                  </div>
                )}
              </div>
            );
