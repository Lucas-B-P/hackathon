...
import { Bath, Calendar, PawPrint, Plus, Stethoscope, X } from "lucide-react";
import { useNavigate } from "react-router";
import { createPet, getPetHistory, getPets, type Pet, type PetHistoryItem } from "../../services/api";
...
export default function MeusPets() {
  ...
  async function openPet(pet: Pet) { setSelected(pet); setHistory([]); try { const result = await getPetHistory(pet.id); setHistory(result.data); } catch { setHistory([]); } }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {pets.map((pet) => <div key={pet.id} onClick={() => openPet(pet)} className="bg-white rounded-2xl border border-[#e5e7eb] shadow-sm p-6 cursor-pointer hover:border-[#86efac] hover:shadow-md transition-all">
          <div className="text-center mb-4"><div className="w-16 h-16 rounded-2xl bg-[#f0fdf4] flex items-center justify-center text-4xl mx-auto mb-3">{<PawPrint size={32} className="text-[#16a34a]" />}</div><h3 className="font-bold text-[#111827] text-lg">{pet.name}</h3><p className="text-sm text-[#6b7280]">{pet.breed || pet.species}</p><p className="text-xs text-[#9ca3af] mt-0.5">{formatDate(pet.birth_date)}</p></div>
      ...
      {selected && <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto"><div className="flex items-center justify-between p-6 border-b border-[#f3f4f6]"><div className="flex items-center gap-3"><div className="w-14 h-14 rounded-xl bg-[#f0fdf4] flex items-center justify-center text-3xl">{<PawPrint size={28} className="text-[#16a34a]" />}</div><div><h2 className="text-xl font-bold text-[#111827]">{selected.name}</h2><p className="text-sm text-[#6b7280]">{selected.breed || selected.species}</p></div></div><button onClick={() => setSelected(null)} className="p-2 hover:bg-[#f3f4f6] rounded-xl text-[#9ca3af]"><X size={20} /></button></div><div className="p-6 space-y-5"><div><h3 className="text-xs font-semibold text-[#9ca3af] uppercase tracking-wide mb-3">Informacoes</h3><div className="grid grid-cols-2 gap-y-2">{[["Sexo", selected.sex || "Nao informado"], ["Nascimento", formatDate(selected.birth_date)], ["Peso", selected.weight ? `${selected.weight} kg` : "Nao informado"]].map(([key, value]) => <div key={key}><p className="text-[11px] text-[#9ca3af]">{key}</p><p className="text-sm font-medium text-[#374151]">{value}</p></div>)}</div></div><div><h3 className="text-xs font-semibold text-[#9ca3af] uppercase tracking-wide mb-3">Historico</h3><div className="space-y-2">{history.length ? history.map((item) => <div key={item.id} className="flex items-center gap-3 p-3 bg-[#f9fafb] rounded-xl"><div className="p-2 rounded-lg bg-[#dcfce7] text-[#16a34a]"><Bath size={14} /></div><div><p className="text-sm font-medium text-[#374151]">{item.type}</p><p className="text-xs text-[#9ca3af]">{item.description} - {formatDate(item.occurred_at)}</p></div></div>) : <p className="text-sm text-[#9ca3af]">Nenhum historico registrado.</p>}</div></div><button onClick={() => navigate("/portal/agendamento")} className="w-full py-3 bg-[#16a34a] hover:bg-[#15803d] text-white font-semibold rounded-xl flex items-center justify-center gap-2"><Calendar size={16} />Agendar servico</button></div></div></div>}
    </div>
  );
}
