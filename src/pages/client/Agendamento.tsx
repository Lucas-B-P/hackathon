...
export default function Agendamento() {
  ...
  const chosenServices = services.filter((item) =>
    selectedServices.includes(item.id),
  )

  const total = chosenServices.reduce(
    (acc, item) => acc + toNumber(item.price),
    0,
  )
    ...
    return (
      <div className="max-w-md mx-auto text-center py-20">
        ...
        <p className="text-[#6b7280] mb-6">
          Seu agendamento foi registrado com sucesso. Até lá!
        </p>
        <div className="bg-[#f0fdf4] rounded-2xl p-5 border border-[#86efac] text-left space-y-2 mb-6">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-[#374151] text-right max-w-[60%]">
              {chosenServices.map((item) => item.name).join(", ")}
            </span>
          </div>
          <div className="flex justify-between text-sm">
    ...
    <div className="max-w-xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-[#6b7280]">
          Siga os passos abaixo para agendar
        </p>
      </div>
      ...
      <div className="flex items-center gap-1">
        {STEPS.map((label, index) => (
          <div key={label} className="flex items-center flex-1 last:flex-none">
            ...
            <span
              className={`text-xs ml-1.5 hidden sm:block ${
                index === step
            ...
            {index < STEPS.length - 1 && (
              <div className="flex-1 h-px bg-[#e5e7eb] mx-2" />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#e5e7eb] shadow-sm p-4 md:p-6">
        {step === 0 && (
          <div className="space-y-3">
            ...
            {pets.length === 0 ? (
              <p className="text-sm text-[#6b7280]">
                Cadastre um pet em Meus Pets antes de agendar.
              </p>
            ) : (
              pets.map((item) => (
                <button
                ...
                >
                  ...
                  {selectedPet === item.id && (
                    <Check size={18} className="ml-auto text-[#16a34a]" />
                  )}
                </button>
              ))
            )}
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${
                      selected ? "bg-[#16a34a]" : "bg-[#f0fdf4]"
                    }`}
                  >
                    <Scissors
                      size={16}
                      className={selected ? "text-white" : "text-[#16a34a]"}
                    />
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#f9fafb] rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="ml-auto text-sm font-semibold text-[#374151]">
                  {pet?.name}
                </span>
              </div>
      ...
      <div className="flex gap-3">
        {step > 0 && (
          <button
            ...
            className="px-5 py-3 border border-[#e5e7eb] rounded-xl text-sm font-medium text-[#374151] hover:bg-[#f3f4f6] transition-colors"
          >
            Voltar
          </button>
        )}
