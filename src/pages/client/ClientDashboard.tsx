    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-[#16a34a] to-[#15803d] rounded-2xl text-white px-5 pt-6 pb-8 md:px-8 md:pt-8 md:pb-10">
        ...
        <p className="text-green-100 mt-2 text-sm md:text-base leading-relaxed">
          Gerencie seus pets, agende serviços e faça compras — tudo em um só
          lugar.
        </p>
      </div>
          ...
          {[
            {
              ...
              sub: upcoming
                ? `${upcoming.service_name} · ${upcoming.pet_name}`
                : "Nenhum agendamento",
...

            {
              ...
              sub: lastOrder
                ? displayOrderStatus(lastOrder.status)
                : "Nenhum pedido",
...

            {
              ...
              sub: appointments[0]
                ? `Último: ${formatDatePt(appointments[0].starts_at)}`
                : "Sem histórico",
            ...
            <div
              key={label}
              className="bg-white rounded-xl border border-[#e5e7eb] shadow-sm p-3 md:p-5"
            >
              <div className={`inline-flex p-2 rounded-xl mb-2 ${color}`}>
                <Icon size={16} />
              </div>
              <p className="text-[10px] md:text-xs text-[#9ca3af] mb-0.5 leading-tight">
                {label}
              </p>
              <p
                className="text-lg md:text-xl font-bold text-[#111827] truncate"
                style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
              >
                {value}
              </p>
              <p className="text-[10px] md:text-xs text-[#9ca3af] mt-0.5 leading-tight truncate">
                {sub}
              </p>
            </div>
          ))}
          ...
          <div className="space-y-3">
            {pets.length === 0 ? (
              <p className="text-sm text-[#9ca3af]">
                Cadastre seu primeiro pet em Meus Pets.
              </p>
            ) : (
              pets.map((pet) => (
                <div
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#9ca3af]">
                      {ag.pet_name} · {formatDatePt(ag.starts_at)}{" "}
                      {formatTimeFromIso(ag.starts_at)}
                    </p>
                  </div>
                  <span
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] shadow-sm p-5 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm font-semibold text-[#111827]">
              #{lastOrder.id} · {displayOrderStatus(lastOrder.status)}
            </p>
          ...
          <p className="text-lg font-bold text-[#111827]">
            R$ {toNumber(lastOrder.total).toFixed(2)}
          </p>
        </div>
      )}
    </div>
