...
export default function ClientLayout() {
  ...
  if (sessionLoading)
    return (
      <div className="min-h-screen bg-[#f9fafb] flex items-center justify-center text-sm text-[#6b7280]">
        Validando sessão...
      </div>
    )
                ...
                <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl border border-[#e5e7eb] shadow-xl z-50 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-[#f3f4f6]">
                    ...
                    >
                      Notificações{" "}
                      {unreadCount > 0 && (
                        <span className="ml-1 text-xs bg-[#dcfce7] text-[#16a34a] px-1.5 py-0.5 rounded-full font-bold">
                          {unreadCount}
                        </span>
                      )}
                    </span>
                    ...
                    {notifications.map((n) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-start gap-3 px-4 py-3 text-left hover:bg-[#f9fafb] transition-colors ${
                            isUnread ? "bg-[#f0fdf4]" : ""
                          }`}
                        >
                          <div
                            className={`p-2 rounded-xl flex-shrink-0 mt-0.5 ${color}`}
                          >
                            <Icon size={14} />
                          </div>
