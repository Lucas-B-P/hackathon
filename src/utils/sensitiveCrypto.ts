...
function resolveKeyMaterial() {
  ...
  if (configured) {
    ...
    if (raw.length !== 32) {
      throw new Error("VITE_SENSITIVE_DATA_KEY deve ser uma string base64 de 32 bytes");
    }
    return raw;
  }
...
async function importKey() {
  return crypto.subtle.importKey("raw", resolveKeyMaterial(), { name: "AES-GCM" }, false, ["encrypt", "decrypt"]);
}
...
export async function decryptProfilePayload<T extends { cpf?: string; phone?: string; birth_date?: string }>(profile: T) {
  return {
    ...profile,
    cpf: profile.cpf ? await decryptSensitive(profile.cpf) : profile.cpf,
    phone: profile.phone ? await decryptSensitive(profile.phone) : profile.phone,
    birth_date: profile.birth_date ? await decryptSensitive(profile.birth_date) : profile.birth_date,
  };
}
